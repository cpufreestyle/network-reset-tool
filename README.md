...
[下载 exe](https://gitee.com/cpufreestyle/network-reset-tool/releases) · [报告问题](https://gitee.com/cpufreestyle/network-reset-tool/issues) · [使用说明](#使用方法)
...
- **根因**：重置面板误引用了诊断类的 `_decode_output` 方法，适配器检测必然抛异常，导致所有 DNS 预设切换永远提示"未找到活动网卡"
- **修复**：解码逻辑抽取为公共函数 `decode_output()`，适配器检测改用独立的 `get_active_adapter()`

### 修复：线程安全与稳定性

- 工作线程不再直接创建 Tkinter 控件（违反 Tk 线程模型，偶发闪退），改为"后台计算 + 主线程渲染"模式
- 健康报告在 Ping 成功但平均延迟为空时崩溃的问题
- 自定义 Ping 按钮在诊断运行期间未禁用的问题
- "重启电脑"按钮实际不执行的问题（PowerShell 参数错误，改用 `shutdown /r /t 5`）

### 安全加固

- 自定义 Ping / Traceroute 目标地址严格校验（仅允许合法域名/IP），杜绝命令注入
- 自定义 DNS 增加 IPv4 数值范围校验（如拒绝 `256.1.1.1`）

### 其他改进

- `reset_tcpip` 如实返回执行结果；无网关的静态 IP 恢复命令不再出现语法错误
- 版本号统一由 `nettoolbox.__version__` 管理
- 代码模块化重构：单文件拆分为 `core`（逻辑）/ `ui`（面板）/ `app`（主窗口），核心逻辑可独立测试
- 仓库清理：移除历史 exe 与临时文件，`*.exe` 等纳入 `.gitignore`
...
1. 下载 [网络工具箱.exe](https://gitee.com/cpufreestyle/network-reset-tool/releases)
...
network-reset-tool/
  ...
  network_reset_gui.py      # Windows GUI 入口 (v3.2)
  ...
  auto_updater.py           # Gitee Release 自动更新模块



  version_info.txt          # PyInstaller 版本资源
